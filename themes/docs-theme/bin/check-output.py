...
def check_featured_image_contract(hugo: str) -> list[str]:
    ...
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="docs-output-featured-") as temp:
        site = Path(temp)
...
""",
        ...
        shutil.copyfile(FIXTURE / "static/images/docs.webp", raster)
        write(
...
def check_theme_color_contract(hugo: str) -> list[str]:
    ...
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="docs-output-theme-color-") as temp:
        site = Path(temp)
...
def check_config_image_policy(hugo: str) -> list[str]:
    ...
    for name, overrides, expected in cases:
        with tempfile.TemporaryDirectory(prefix=f"docs-output-{name}-") as temp:
