...
def check_generic_rss_output(hugo: str) -> list[str]:
    ...
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="docs-code-generic-rss-") as temp:
        site = Path(temp)
...
def check_nested_render_ids(hugo: str) -> list[str]:
    ...
    result, destination, temp = temp_build(
        hugo, pages, prefix="docs-code-nested-ids-", extra_config=config
    )
    with temp:
        if result.returncode != 0:
...
def check_invalid_cases(hugo: str) -> list[str]:
    ...
    for name, body, expected in INVALID_CASES:
        with tempfile.TemporaryDirectory(prefix=f"docs-code-{name}-") as temp:
            temp_path = Path(temp)
