...
The point is not tidiness. `.leaf`, `.is-open`, `data-url`, and `--docs-ink`
...
# docs.pgsty.com/docs/design/components/ owns the public marker namespace.
AUTHOR_MARKERS = {
...
# leaves the player on its built-in black-on-white defaults inside an Docs
...
def check_sources() -> list[str]:
    ...
    for name in sorted(globals_set):
        if not name.startswith("Docs") and name not in {"HTMLDialogElement"}:
            errors.append(f"window.{name} is outside the Docs* global namespace")
    ...
    # asciinema's built-in black inside a light Docs window — the 0.5 break.
    scss = "\n".join(
...
def main() -> int:
    ...
    if errors:
        ...
        for error in errors:
            print(f"  {error}")
        return 1
    print("Namespace checks passed (td- classes, data-td- attributes, --td- properties, Docs* globals)")
    return 0
