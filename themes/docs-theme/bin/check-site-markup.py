#!/usr/bin/env python3
"""Validate the resolved Goldmark settings required by Docs native forms.
...
def main() -> int:
    ...
    if missing:
        print(f"Docs Goldmark preflight failed for {site}:")
        for label in missing:
            print(f"  required: {label}")
        return 1

    print(f"Docs Goldmark preflight passed: {site}")
    return 0
