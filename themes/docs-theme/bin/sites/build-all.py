...
copies the current tree. A scratch ``go.work`` replaces Docs without modifying
...
def build(snapshot: Path, hugo: str) -> dict:
    ...
    cmd = [
        ...
        "--ignoreVendorPaths", "github.com/pgsty/docs",
    ]
...
def main() -> int:
    ...
    work = args.keep.resolve() if args.keep else Path(tempfile.mkdtemp(prefix="docs-sites-"))
