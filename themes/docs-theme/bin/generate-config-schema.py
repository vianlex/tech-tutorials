...
def load_check_params():
    spec = importlib.util.spec_from_file_location("docs_check_params", ROOT / "bin/check-params.py")
...
def build(check_params) -> dict[str, str]:
    ...
    site_document = {
        ...
        "$comment": stamp,
        "title": "Docs site configuration (hugo.yaml)",
        "type": "object",
        "additionalProperties": True,
        "properties": {"params": {**params_schema, "description": "Docs theme parameters."}},
    }
    front_document = {
        "$schema": "http://json-schema.org/draft-07/schema#",
        "$comment": stamp,
        "title": "Docs page front matter",
        **front_schema,
    }
    return {
