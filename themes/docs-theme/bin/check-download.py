...
def check_example(public: Path) -> list[str]:
    ...
    for marker in (
        ...
        "https://github.com/pgsty/docs/archive/refs/tags/v2.4.1.tar.gz",
        "https://github.com/pgsty/docs/releases/download/v2.4.1/demo-2.4.1-linux-amd64.tar.gz",
    ):
        require(marker in demo, f"published download fixture lost {marker}", errors)
    for marker in (
        "curl -fsSL https://repo.example.org/demo | bash",
        "git clone --branch v2.4.1 https://github.com/pgsty/docs.git",
    ):
    ...
    for marker in (
        "Pending release",
        'aria-disabled="true"',
        "docs-3.0.0-rc.1.tar.gz",
    ):
        require(marker in pending, f"unpublished download fixture lost {marker}", errors)
    require(
        "curl -fsSL https://repo.example.org/docs/install | bash" in pending_text,
    ...
    require(
        'href="https://repo.example.org/docs/"' in pending,
    ...
    require("docs\\-3\\.0\\.0\\-rc\\.1\\.tar\\.gz" in pending_md, "unpublished Markdown lost asset identity", errors)

    for name, source_marker in (
        ("download-demo", "git clone --branch v2.4.1"),
        ("download-pending", "curl -fsSL https://repo.example.org/docs/install"),
    ):
...
def check_site_version_fallback(hugo: str) -> list[str]:
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="docs-components-download-version-") as temp:
        site = Path(temp)
        create_site(
            site,
            """repo: pgsty/docs
...
def check_rss(hugo: str) -> list[str]:
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="docs-components-download-rss-") as temp:
        site = Path(temp)
        create_site(
            site,
            """version: 1.0.0
...
def check_invalid(hugo: str) -> list[str]:
    ...
    for name, data, body, expected in INVALID_CASES:
        with tempfile.TemporaryDirectory(prefix=f"docs-components-download-{name}-") as temp:
            site = Path(temp)
...
def main() -> int:
    ...
    if args.public is None:
        with tempfile.TemporaryDirectory(prefix="docs-components-download-example-") as temp:
