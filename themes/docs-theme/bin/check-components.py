...
def check_outputs(public: Path) -> list[str]:
    ...
    require("DocsEchartsFunctions" in source and "data-td-echarts" in source, "data-fence chunks lack the ECharts runtime", errors)
...
def check_callout_matrix(hugo: str) -> list[str]:
    ...
    result, destination, temp = build_site(
        ...
        prefix="docs-components-callouts-",
    )
    with temp:
...
def check_openapi(hugo: str) -> list[str]:
    ...
    result, destination, temp = build_site(
        ...
        prefix="docs-components-openapi-",
    )
    with temp:
        if result.returncode != 0:
    ...
    for name, body, expected in invalid:
        ...
        with temp:
            ...
            strict, _strict_destination, strict_temp = build_site(
                hugo,
                {"docs/_index.md": "---\ntitle: Docs\n---\n", "docs/bad.md": f"---\ntitle: {name}\n---\n\n{body}\n"},
                prefix=f"docs-components-{name}-strict-",
...
def check_media_output_states(hugo: str) -> list[str]:
    ...
    result, destination, temp = build_site(
        hugo,
        {"docs/_index.md": "---\ntitle: Docs\n---\n", "docs/media.md": page},
        prefix="docs-components-media-states-",
...
def check_template_contracts() -> list[str]:
    ...
    require("$fn:" in runtime and "DocsEchartsFunctions" in runtime, "ECharts callback bridge ($fn: / DocsEchartsFunctions) changed", errors)
    ...
    require("docs:" not in config, "theme hugo.yaml declares a params.docs tree", errors)
