...
def temp_page_build(hugo: str, body: str, *, front: str = "", panic_on_warning: bool = False) -> tuple[subprocess.CompletedProcess[str], str, str]:
    with tempfile.TemporaryDirectory(prefix="docs-gallery-page-") as temp:
        temp_path = Path(temp)
...
def check_subpath(hugo: str) -> list[str]:
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="docs-gallery-subpath-") as temp:
...
def check_rss_output(hugo: str) -> list[str]:
    ...
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="docs-gallery-rss-") as temp:
        site = Path(temp)
...
def check_zoom_reuse(hugo: str) -> list[str]:
    ...
    for name, site_enabled, page_enabled, body, expected in cases:
        with tempfile.TemporaryDirectory(prefix=f"docs-gallery-zoom-{name}-") as temp:
            site = Path(temp)
