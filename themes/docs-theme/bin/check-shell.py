...
def build_example(hugo: str) -> list[str]:
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="docs-shell-") as temporary:
        ...
        if page.is_file():
            ...
            require(any("DocsFeedback" in bundle for bundle in bundles),
        ...
        if article.is_file():
            ...
            # `authors: [docs, vonng]` -- GetTerms keeps the front matter
            # sequence, so the project author bylines first because the page
            # says so, not because of any sort.
            order = [article_source.find('href="/authors/docs/"'),
        ...
        for slug, label in (("vonng", "Ruohang Feng"), ("docs", "Docs")):
        ...
        if feed.is_file():
            ...
            require('xmlns:dc="http://purl.org/dc/elements/1.1/"' in feed_source
                    ...
                    and "<dc:creator>Docs</dc:creator>" in feed_source,
...
def build_footer_utilities_fixture(hugo: str) -> list[str]:
    ...
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="docs-shell-footer-") as temporary:
        root = Path(temporary)
        source = root / "site"
        public = root / "public"
...
def build_self_root_fixture(hugo: str) -> list[str]:
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="docs-shell-self-root-") as temporary:
        root = Path(temporary)
        source = root / "site"
        public = root / "public"
...
def build_featured_image_rejection(hugo: str) -> list[str]:
    ...
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="docs-shell-featured-") as temporary:
        root = Path(temporary)
        source = root / "site"
...
def build_navbar_menu_columns(hugo: str) -> list[str]:
    ...
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="docs-shell-navbar-") as temporary:
        root = Path(temporary)
        source = root / "site"
        (source / "content/docs").mkdir(parents=True)
        (source / "hugo.yaml").write_text(
...
def build_blog_index_forms(hugo: str) -> list[str]:
    ...
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="docs-blog-index-") as temporary:
        root = Path(temporary)
        ...
        for name, extra in (("rows", (rows_overlay,)), ("cards", (overlay,)),
            ...
            index = public / "blog/docs/index.html"
            if not index.is_file():
                return [f"blog index {name} form did not render blog/docs/index.html"]
        ...
        require(not (root / "table/blog/docs/page").exists(),
        ...
        second_path = root / "toggle/blog/docs/page/2/index.html"
...
def build_blog_card_fixture(hugo: str) -> list[str]:
    ...
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="docs-blog-card-") as temporary:
        root = Path(temporary)
        source = root / "site"
        public = root / "public"
...
def build_share_fixture(hugo: str) -> list[str]:
    ...
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="docs-shell-share-") as temporary:
        root = Path(temporary)
        source = root / "site"
        public = root / "public"
        (source / "content/blog").mkdir(parents=True)
