...
def check_pager_outputs(public: Path) -> list[str]:
    ...
    blog_cases = {
        ...
        "blog/typography/index.html": {"prev": "/blog/", "next": "/blog/docs/"},
        "blog/docs/index.html": {"prev": "/blog/typography/", "next": "/blog/docs/immersive-reading/"},
        "blog/docs/immersive-reading/index.html": {
            "prev": "/blog/docs/",
            "next": "/blog/docs/docs-announcement/",
        },
        "blog/older/index.html": {
            "prev": "/blog/docs/docs-implementation-diary/",
...
def check_explicit_navigation(hugo: str) -> list[str]:
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="docs-components-explicit-nav-") as temp:
        source = Path(temp)
...
def check_home_root_navigation(hugo: str) -> list[str]:
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="docs-components-home-root-") as temp:
        source = Path(temp)
        (source / "themes").mkdir(parents=True)
        (source / "themes/docs").symlink_to(ROOT, target_is_directory=True)
        write_file(
            source / "hugo.yaml",
            """baseURL: https://example.org/
...
def check_invalid_pager_config(hugo: str) -> list[str]:
    ...
    for name, front_matter, config_extra, expected in cases:
        with tempfile.TemporaryDirectory(prefix=f"docs-components-pager-{name}-") as temp:
            source = Path(temp)
            create_theme_site(source)
...
def check_invalid_eq_escape(hugo: str) -> list[str]:
    ...
    for name, body, expected in cases:
        with tempfile.TemporaryDirectory(prefix=f"docs-components-eq-{name}-") as temp:
            source = Path(temp)
            create_theme_site(source)
            write_file(
                source / "content/docs/page.md",
...
def check_rss_pager_output(hugo: str) -> list[str]:
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="docs-components-pager-rss-") as temp:
        source = Path(temp)
        create_theme_site(source)
        config_path = source / "hugo.yaml"
        config = config_path.read_text()
...
def main() -> int:
    ...
    if args.public is not None:
        ...
        with tempfile.TemporaryDirectory(prefix="docs-components-reading-") as temp:
