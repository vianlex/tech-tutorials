...
def check_example(public: Path) -> list[str]:
    ...
    for marker in (
        ...
        'class="td-shell-footline__center">Powered by <a href="https://docs.pgsty.com">Docs</a>',
    ...
    require("DocsLanding" in source, "landing runtime source is missing", errors)
...
def check_sources() -> list[str]:
    ...
    for marker in (
        ...
        "DocsSurfaceCoordinator", "register('mobile-menu'",
    ):
...
"""
    with tempfile.TemporaryDirectory(prefix="docs-landing-centered-hero-") as temp:
        site = Path(temp)
        create_site(site, data)
        result = run(hugo, site)
        if result.returncode != 0:
...
{indented}
"""
    with tempfile.TemporaryDirectory(prefix="docs-landing-preview-") as temp:
        site = Path(temp)
        create_site(site, data)
