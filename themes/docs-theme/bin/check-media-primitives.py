...
def check_outputs(public: Path) -> list[str]:
    ...
    for marker in (
        ...
        '<small class="td-figure__byline">Docs fixture byline</small>',
...
def check_resolver_matrix(hugo: str) -> list[str]:
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="docs-media-resolver-") as temp:
        temp_path = Path(temp)
...
def check_image_hook_matrix(hugo: str) -> list[str]:
    ...
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="docs-media-hook-") as temp:
        temp_path = Path(temp)
...
def check_subpath(hugo: str) -> list[str]:
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="docs-media-subpath-") as temp:
...
def check_rss_output(hugo: str) -> list[str]:
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="docs-media-rss-") as temp:
        temp_path = Path(temp)
...
def check_generic_rss_output(hugo: str) -> list[str]:
    ...
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="docs-media-generic-rss-") as temp:
        site = Path(temp)
...
def check_invalid_cases(hugo: str) -> list[str]:
    ...
    for name, body, expected in INVALID_CASES + HOOK_INVALID_CASES:
        with tempfile.TemporaryDirectory(prefix=f"docs-media-{name}-") as temp:
            temp_path = Path(temp)
