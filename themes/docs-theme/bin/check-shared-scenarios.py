...
def check_development_robots(hugo: str) -> list[str]:
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="docs-components-robots-") as temp:
...
def check_invalid_config(hugo: str) -> list[str]:
    ...
    for name, value, expected in cases:
        with tempfile.TemporaryDirectory(prefix=f"docs-components-invalid-{name}-") as temp:
            temp_path = Path(temp)
    ...
    with tempfile.TemporaryDirectory(prefix="docs-components-invalid-search-serve-") as temp:
        temp_path = Path(temp)
        override = temp_path / "override.yaml"
    ...
    with tempfile.TemporaryDirectory(prefix="docs-components-search-hook-") as temp:
        temp_path = Path(temp)
...
def check_navbar_autohide(hugo: str) -> list[str]:
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="docs-navbar-autohide-") as temp:
        temp_path = Path(temp)
        source = temp_path / "site"
...
def check_lastmod_output(hugo: str) -> list[str]:
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="docs-components-lastmod-") as temp:
        source = Path(temp)
...
def check_stable_404(hugo: str) -> list[str]:
    errors: list[str] = []
    digests: set[str] = set()
    with tempfile.TemporaryDirectory(prefix="docs-components-404-") as temp:
...
def main() -> int:
    ...
    if args.public is None:
        with tempfile.TemporaryDirectory(prefix="docs-components-misc-") as temp:
            public = Path(temp) / "public"
