...
scratch ``go.work`` replacement and ignore only a vendored Docs copy, ensuring
...
def prepare_workspace(snapshot: Path, theme: Path) -> None:
    ...
    subprocess.run(["go", "work", "edit", f"-replace=github.com/pgsty/docs={theme}"], cwd=snapshot,
                   check=True, capture_output=True)
...
def measure_build(name: str, snapshot: Path, hugo: str, theme: Path, *, fixture: bool,
    ...
    if fixture:
        ...
        extra += ["--ignoreVendorPaths", "github.com/pgsty/docs"]
    if config_extra:
...
def render_markdown(report: dict) -> str:
    out: list[str] = []
    out.append(f"# Docs T00 baseline — {report['generated_at']}")
    out.append("")
    theme = report["theme"]
...
def main() -> int:
    ...
    work_root = Path(args.keep).resolve() if args.keep else Path(tempfile.mkdtemp(prefix="docs-baseline-"))
