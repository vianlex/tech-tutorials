...
def check_home_manifest(hugo: str) -> list[str]:
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="docs-components-book-home-manifest-") as temp:
        source = Path(temp)
        write(
            source / "hugo.yaml",
...
def check_example(public: Path) -> list[str]:
    ...
    if one and two and three and four and root:
        ...
        for document, target_id, image_src in (
            (one, "fig-overview", "/images/docs.webp"),
            (two, "fig-release", "/images/releasenote.webp"),
            (three, "fig-operations", "/images/docs.webp"),
        ):
    ...
    for marker in (
        "**Figure 1-1.**",
        "![Docs documentation site overview](/images/docs.webp)",
        "**Table 1-1.**",
        "**Equation 1.1.**",
        "**Example 1-1.**",
...
def check_toc_heading_entities(hugo: str) -> list[str]:
    ...
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="docs-components-book-toc-entities-") as temp:
        source = Path(temp)
...
def check_reading_time(hugo: str) -> list[str]:
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="docs-components-book-reading-time-") as temp:
        source = Path(temp)
...
def check_invalid_components(hugo: str) -> list[str]:
    ...
    for name, body, expected in cases:
        with tempfile.TemporaryDirectory(prefix=f"docs-components-book-invalid-{name}-") as temp:
            source = Path(temp)
    ...
    for name, extra_ui, extra_params, draft, expected in config_cases:
        with tempfile.TemporaryDirectory(prefix=f"docs-components-book-config-{name}-") as temp:
            source = Path(temp)
...
def check_footnotes(hugo: str) -> list[str]:
    ...
    with tempfile.TemporaryDirectory(prefix="docs-components-book-footnote-") as temp:
        source = Path(temp)
        create_site(source, body)
        result = build(hugo, source, source / "public")
        if result.returncode != 0:
...
def check_invalid_contributors(hugo: str) -> list[str]:
    ...
    for name, data, expected in cases:
        with tempfile.TemporaryDirectory(prefix=f"docs-components-contributors-invalid-{name}-") as temp:
            source = Path(temp)
...
def check_ddia_compatibility(hugo: str) -> list[str]:
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="docs-components-book-ddia-") as temp:
        source = Path(temp)
        create_site(
            source,
...
def check_rss_output(hugo: str) -> list[str]:
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="docs-components-book-rss-") as temp:
        source = Path(temp)
        write(
            source / "hugo.yaml",
...
def check_publication_helpers() -> list[str]:
    ...
    try:
        epub = load("docs_book_epub", "bin/book-epub.py")
        pdf = load("docs_book_pdf", "bin/book-pdf.py")
        ...
        with tempfile.TemporaryDirectory(prefix="docs-publication-safety-") as temp:
            root = Path(temp)
...
def main() -> int:
    ...
    if args.public is None:
        with tempfile.TemporaryDirectory(prefix="docs-components-book-") as temp:
