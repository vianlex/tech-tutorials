...
def create_site(root: Path, config_extra: str = "") -> None:
    ...
    (root / "themes/docs").symlink_to(ROOT, target_is_directory=True)
    write(
        root / "hugo.yaml",
...
def check_example(public: Path) -> list[str]:
    ...
    require(
        'href="https://downloads.example.org/releases/stable/Docs%20manual%20%28final%29.zip"'
        in base,
    ...
    if asset_runtime and asset_runtime.path.is_file():
        require("DocsAssetList" in asset_runtime.path.read_text(encoding="utf-8"), "asset-list runtime was not bundled", errors)
...
def check_releases_layout(hugo: str) -> list[str]:
    ...
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="docs-components-release-index-") as temp:
        site = Path(temp)
        create_site(site)
        write(
        ...
        records = (
            ("docs-five", "https://github.com/pgsty/docs/releases/tag/v0.5.0", "2026-08-18"),
        ...
        expected = [
            ("/blog/releases/docs-five/", "docs v0.5.0"),
...
def check_algorithms_and_rss(hugo: str) -> list[str]:
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="docs-components-release-formats-") as temp:
        site = Path(temp)
        create_site(site)
        blocks = (
            ("md5", "a" * 32, "MD5"),
    ...
    with tempfile.TemporaryDirectory(prefix="docs-components-release-rss-") as temp:
        site = Path(temp)
        create_site(site)
...
date: 2026-08-14
outputs: [RSS]
release_url: https://github.com/pgsty/docs/releases/tag/v2.0.0
---

{{< release-card >}}

{{< release-assets >}}
eeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee  docs.zip
{{< /release-assets >}}
""",
        )
        result = run(hugo, site)
        if result.returncode != 0:
            ...
            if outputs:
                ...
                for marker in (
                    "https://github.com/pgsty/docs/releases/tag/v2.0.0",
                    "| File | Checksum |",
                    "e" * 64,
                ):
...
def check_removed_list_keys(hugo: str) -> list[str]:
    ...
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="docs-components-release-removed-") as temp:
        site = Path(temp)
        create_site(site)
        write(
            site / "content/blog/releases/_index.md",
...
INVALID_CASES = (
    (
        ...
        "release_url: https://gitlab.com/pgsty/docs/releases/tag/v1.0.0",
        "{{< release-card >}}",
    ...
    ("release-legacy-map", "release:\n  version: 1.0.0\n  repo: pgsty/docs", "{{< release-card >}}", "was replaced by `release_url`"),
    ("release-legacy-shorthand", "release: https://github.com/pgsty/docs/releases/tag/v1.0.0", "{{< release-card >}}", "was replaced by `release_url`"),
    ("release-card-param", "release_url: https://github.com/pgsty/docs/releases/tag/v1.0.0", '{{< release-card version="1.0.0" >}}', "accepts no parameters"),
    ...
    ("asset-release-base", "release_url: https://github.com/pgsty/docs/releases/tag/v1.0.0", '{{< release-assets base="/files" >}}\n' + "a" * 64 + "  file.zip\n{{< /release-assets >}}", "base is only valid without release_url front matter"),
...
def check_invalid(hugo: str) -> list[str]:
    ...
    for name, front_matter, body, expected in INVALID_CASES:
        with tempfile.TemporaryDirectory(prefix=f"docs-components-{name}-") as temp:
            site = Path(temp)
            create_site(site)
            write(
...
def main() -> int:
    ...
    if args.public is None:
        with tempfile.TemporaryDirectory(prefix="docs-components-release-example-") as temp:
