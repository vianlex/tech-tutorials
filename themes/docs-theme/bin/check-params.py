...
Docsy front matter key that predates Docs.
...
def build_case(hugo: str, name: str, params: str, front_matter: str,
    ...
    with tempfile.TemporaryDirectory(prefix=f"docs-params-{name}-") as temp:
        source = Path(temp)
...
def check_blog_index_enum(hugo: str) -> list[str]:
    ...
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="docs-params-blog-index-") as temp:
        source = Path(temp)
...
def check_blog_numeric_params(hugo: str) -> list[str]:
    ...
    for name, params, expected in cases:
        with tempfile.TemporaryDirectory(prefix=f"docs-params-blog-{name}-") as temp:
            source = Path(temp)
            (source / "content/blog").mkdir(parents=True)
            (source / "hugo.yaml").write_text(site_config(params), encoding="utf-8")
            (source / "content/blog/_index.md").write_text(
    ...
    with tempfile.TemporaryDirectory(prefix="docs-params-blog-octal-") as temp:
        source = Path(temp)
        (source / "content/blog").mkdir(parents=True)
...
def check_print_params(hugo: str) -> list[str]:
    ...
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="docs-params-print-") as temp:
        source = Path(temp)
        (source / "content/docs").mkdir(parents=True)
