...
def check_subpath(hugo: str) -> list[str]:
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="docs-primitives-subpath-") as temp:
...
def check_rss_output(hugo: str) -> list[str]:
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="docs-primitives-rss-") as temp:
        temp_path = Path(temp)
...
def check_generic_rss_output(hugo: str) -> list[str]:
    ...
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="docs-primitives-generic-rss-") as temp:
        site = Path(temp)
...
def check_rss_summary_contract(hugo: str) -> list[str]:
    ...
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="docs-primitives-rss-summary-") as temp:
        site = Path(temp)
        content = site / "content/docs"
...
def check_field_parity(html: str) -> list[str]:
    ...
    that is what this compares (docs.pgsty.com/docs/design/components/).
    """
    errors: list[str] = []
...
def check_field_anchors(html: str) -> list[str]:
    ...
    collapses to one hyphen (docs.pgsty.com/docs/design/components/).
    """
    errors: list[str] = []
    for name, anchor in (
...
def check_table_family(hugo: str) -> list[str]:
    ...
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="docs-primitives-table-family-") as temp:
        site = Path(temp)
...
def check_invalid_cases(hugo: str) -> list[str]:
    ...
    with tempfile.TemporaryDirectory(prefix="docs-primitives-invalid-") as temp:
        temp_path = Path(temp)
        content = temp_path / "content/docs"
        content.mkdir(parents=True)
    ...
    for name, body, expected in INVALID_CASES:
        if name not in UNBATCHED_INVALID_CASES:
            continue
        with tempfile.TemporaryDirectory(prefix=f"docs-primitives-{name}-") as temp:
            temp_path = Path(temp)
            content = temp_path / "content/docs"
            content.mkdir(parents=True)
...
def check_template_contracts() -> list[str]:
    ...
    for marker in ("DocsFileTree", "module.exports", "data-td-filetree-divider", "pointerdown", "'ArrowRight'", "'Home'", "'End'", "--td-filetree-name-col"):
