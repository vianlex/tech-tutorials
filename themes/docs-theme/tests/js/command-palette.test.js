...
function setup({ controlledAnimationFrame = false } = {}) {
  ...
  global.window = {
    __Docs_PALETTE_MANUAL_INIT__: true,
    clearTimeout() {},
  ...
  global.window.DocsPaletteModel = require(modelPath);
  ...
  const controller = paletteModule.init({ root, registry, model: window.DocsPaletteModel, searchApi });
