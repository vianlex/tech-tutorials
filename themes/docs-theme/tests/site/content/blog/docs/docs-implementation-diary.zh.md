---
title: Docs 实施日记：从复制外壳到统一主题
linkTitle: Docs 实施日记
date: 2026-08-08
aliases: [/blog/2026/docs-implementation-diary/]
lastmod: 2026-08-08
description: >-
  记录 Docs 实现预览背后的技术决策、迁移方法、安全边界、测试与文档工作。
author: Docs 维护者
tags: [Docs]
---

Docs 始于一个令人不安的事实：多个生产文档站之所以看起来相互关联，是因为它们确实源于同一套实现；但公共实现却以复制文件的形式散落在各处。呈现效果足够一致，维护模型却并非如此。
...
这排除了一个看似诱人、实际代价高昂的捷径：增加 `params.docs.enabled`
...
Awesome 来自 npm，部分路径还会调用 PostCSS。Docs 把必需源码与编译产物移入主题，并让 SCSS 留在 Hugo 自身的 Asset
Pipeline 中。
...
PlantUML 与 Diagrams.net 迫使我们做出一项重要区分：它们依赖服务，而不只是 JavaScript 库。Docs 拒绝虚构公共端点；启用功能却没有配置服务时，构建会失败。
...
## 把样例站变成 Docs 文档 {#turning-the-sample-into-docs-docs}
...
4. 增加 Docs 产品指南、项目公告与本实施日记。

翻译之前，我们先建立术语与排版指南。随后使用检查器验证源文件与译文配对、标题数量、中文显式 ID，以及渲染后的中英文标题 ID 是否完全相同。

Docsy 历史发布文章保持忠实翻译，其中的 npm 时代说明属于历史语境；Docs 架构与迁移指南则明确说明当前仅依赖 Hugo 的产品契约。
