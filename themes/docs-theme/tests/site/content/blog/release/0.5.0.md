---
title: Docs 0.5.0 — Component API v5 and the converged configuration
linkTitle: Docs v0.5.0
date: 2026-08-18T00:00:00+08:00
lastmod: 2026-08-18T00:00:00+08:00
draft: false
description: >-
  Docs 0.5.0 replaces most shortcodes with native Markdown forms, converges
...
authors: [vonng]
tags: [Docs, Release]
release_url: https://github.com/pgsty/docs/releases/tag/v0.5.0
series: [docs-releases]
series_weight: 5
---

Docs 0.5.0 is the API-freeze release. It contains the change set that the 1.0
...
- **Content**: most components are written as Markdown — `> [!TYPE]` callouts,
  ...
  are new; 29 remain as full forms. `scripts/migrations/docs06.py` rewrites
  content.
...
- **Namespace**: theme classes are `td-*`, data attributes `data-td-*`, custom
  properties `--td-*`, JS globals `Docs*`; the `docs-*` set and Docsy leftovers
...
- **Migration**: `docs06.py report → migrate --write → check` for content and
...
`mermaid plantuml markmap math chem echarts infographic checksums filetree gallery`; `echarts` is declarative, `$fn:<name>` callbacks come from `window.DocsEchartsFunctions`.
{{< /field >}}
...
The chip on each entry is the toolkit key (`scripts/migrations/docs06.py
...
0.5.0: same-named data fences; `$fn:` callbacks are unchanged, `js` sub-fences move to `window.DocsEchartsFunctions`.
{{< /field >}}
{{< field name="doc-cards · doc-card · nav-cards · nav-card · card · cardpane · doc-carousel" type="cards" >}}
0.4: the Docsy card family and the Docs `doc-cards` / `nav-cards` wrappers.
...
`scripts/migrations/docs06.py migrate --only frontmatter` rewrites all of the
...
| Kind | 0.4 | 0.5.0 |
| --- | --- | --- |
| Classes | `docs-*` (landing subsystem), `leaf`, `has-child`, `active-path`, `is-open`, `is-active`, `is-hidden`, `is-disabled`, `landing-header`, `landing-nav`, `landing-container`, `article-meta`, `pageinfo`, `nav-*`, `taxonomy-*`, `ul-N` | `td-*` throughout; the site header and nav are `td-site-header`, `td-site-nav`, `td-site-container` |
| Data attributes | `data-docs-*` | `data-td-*` |
| Custom properties | `--docs-*`, `--term-*` | `--td-*` |
| JS globals | `docs*` / `echartsFunctions` | `window.DocsActions`, `DocsEchartsFunctions`, `DocsLanding`, `DocsSearchEngine`, `DocsSurfaceCoordinator` |
...
- `static/css/giscus-docs-{light,dark}.css` are gone. The palettes ship as
...
```sh
python3 path/to/docs/scripts/check-site-markup.py --site ~/pgsty/example.com
```

### 1. Inventory {#migration-inventory}

```sh
python3 scripts/migrations/docs06.py report --sites ~/pgsty/example.com --md report.md --json report.json
```
...
```sh
python3 scripts/migrations/docs06.py migrate --site ~/pgsty/example.com          # dry run: diffs + counts
python3 scripts/migrations/docs06.py migrate --site ~/pgsty/example.com --write  # atomic rewrite
python3 scripts/migrations/docs06.py migrate --site ~/pgsty/example.com --write  # second run: changed 0
python3 scripts/migrations/docs06.py check   --site ~/pgsty/example.com          # residual legacy syntax → exit 1
```
...
`window.DocsEchartsFunctions` entries.
...
- `assets/scss/_styles_project.scss`: remove `@import 'td/color-adjustments-dark'`,
  ...
  `.td-navbar-transparent`, `.td-box*`, `-bg-*`, `docs-*`, `--docs-*`.
...
- Site JS and tests: `docs-*` ids and `data-docs-*` attributes are `td-*` /
...
```sh
hugo mod get github.com/pgsty/docs@v0.5.0
hugo mod tidy
```
...
- The module path remains `github.com/pgsty/docs`; consumers still need no
...
[v0.4.2 to v0.5.0](https://github.com/pgsty/docs/compare/v0.4.2...v0.5.0) and
