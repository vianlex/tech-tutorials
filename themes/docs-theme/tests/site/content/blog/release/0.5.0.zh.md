---
title: Docs 0.5.0 — 组件 API v5 与收敛后的配置
linkTitle: Docs v0.5.0
date: 2026-08-18T00:00:00+08:00
lastmod: 2026-08-18T00:00:00+08:00
draft: false
description: >-
  Docs 0.5.0 用原生 Markdown 形态取代大多数 shortcode，把全部配置键与 front matter
...
Docs 0.5.0 是 API 冻结版本。它包含 1.0 线将要冻结的全部变更：组件 API v5
...
- **内容**：大多数组件直接用 Markdown 写——`> [!TYPE]` 提示块、`{.steps}` /
  ...
  剩下 29 个作为完整形态。`scripts/migrations/docs06.py` 负责改写内容。
...
- **命名空间**：主题 class 一律 `td-*`、data 属性 `data-td-*`、自定义属性 `--td-*`、
  JS 全局 `Docs*`；`docs-*` 一族和 Docsy 遗留（`leaf`、`has-child`、`nav-*` …）
...
- **迁移**：内容与 front matter 走 `docs06.py report → migrate --write → check`，
  然后构建——报错就是配置清单。
...
`mermaid plantuml markmap math chem echarts infographic checksums filetree gallery`；`echarts` 只做声明式配置，`$fn:<name>` 回调来自 `window.DocsEchartsFunctions`。
{{< /field >}}
...
每个条目上的 chip 是迁移工具键（`scripts/migrations/docs06.py migrate --only <key>`）；
...
0.5.0：同名数据围栏；`$fn:` 回调不变，`js` 子围栏改到 `window.DocsEchartsFunctions`。
{{< /field >}}
{{< field name="doc-cards · doc-card · nav-cards · nav-card · card · cardpane · doc-carousel" type="cards" >}}
0.4：Docsy 的卡片族与 Docs 的 `doc-cards` / `nav-cards` 包装。
...
`scripts/migrations/docs06.py migrate --only frontmatter` 会改写以上全部页面键，
...
| 类别 | 0.4 | 0.5.0 |
| --- | --- | --- |
| class | `docs-*`（landing 子系统）、`leaf`、`has-child`、`active-path`、`is-open`、`is-active`、`is-hidden`、`is-disabled`、`landing-header`、`landing-nav`、`landing-container`、`article-meta`、`pageinfo`、`nav-*`、`taxonomy-*`、`ul-N` | 全部 `td-*`；站点页头与导航是 `td-site-header`、`td-site-nav`、`td-site-container` |
| data 属性 | `data-docs-*` | `data-td-*` |
| 自定义属性 | `--docs-*`、`--term-*` | `--td-*` |
| JS 全局 | `docs*` / `echartsFunctions` | `window.DocsActions`、`DocsEchartsFunctions`、`DocsLanding`、`DocsSearchEngine`、`DocsSurfaceCoordinator` |
...
- `static/css/giscus-docs-{light,dark}.css` 没有了。调色板以
...
```sh
python3 scripts/migrations/docs06.py migrate --site ~/pgsty/example.com          # dry run：diff + 计数
python3 scripts/migrations/docs06.py migrate --site ~/pgsty/example.com --write  # 原子改写
python3 scripts/migrations/docs06.py migrate --site ~/pgsty/example.com --write  # 第二次：changed 0
python3 scripts/migrations/docs06.py check   --site ~/pgsty/example.com          # 残留旧语法 → exit 1
```
...
以及必须改成 `window.DocsEchartsFunctions` 条目的 `echarts` `js` 子围栏。
...
- 站点 JS 与测试：`docs-*` id 与 `data-docs-*` 属性改为 `td-*` / `data-td-*`；
...
- 模块路径仍是 `github.com/pgsty/docs`；消费站点仍不需要 Node.js、CDN 或构建期下载。
...
完整源码差异见 [v0.4.2 到 v0.5.0](https://github.com/pgsty/docs/compare/v0.4.2...v0.5.0)
与主题的 `CHANGELOG.md`。
