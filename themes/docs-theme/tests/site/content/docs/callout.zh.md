...
前五种与 GitHub 一致，后五种是 Docs 追加的语义类型。每种类型有默认图标与强调色。
...
> [!QUESTION]
> 站点要不要开评论？看[启用评论](https://docs.pgsty.com/zh/docs/admin/comments/)。
...
> [!TIP] 三条命令启动预览
>
> 1. 克隆：`git clone https://github.com/pgsty/docs.pgsty.com my-docs`
> 2. 进入目录并预览：
>    ```bash
...
- 折叠状态不持久化。
- 提示块可以放在 `{.steps}` 列表项与 `{{%/* steps */%}}` 步骤中（见[步骤](/zh/docs/steps/)），块引用的每一行都以 `>` 开头，缩进与列表项对齐。

## 相关 {#related}

- [步骤](/zh/docs/steps/) — 步骤中放置提示块
- [标签页](/zh/docs/tabs/) — 同一提示按平台分开呈现
- [编写页面](https://docs.pgsty.com/zh/docs/write/pages/) — 提示块与正文的取舍
