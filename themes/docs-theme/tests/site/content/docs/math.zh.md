...
`xref` 可以写在目标之前，前向引用合法。整本书的公式目录、`book-equations` 索引见[书籍出版](https://docs.pgsty.com/zh/docs/write/book/)。
...
各键的完整定义见[配置总览](https://docs.pgsty.com/zh/docs/customize/config/)。分隔符不能与站点正文冲突：单个 `$` 没有配进去，避免「$5」这样的价格被当成公式。
...
- [书籍出版](https://docs.pgsty.com/zh/docs/write/book/) — 公式目录与跨页交叉引用
- [配置总览](https://docs.pgsty.com/zh/docs/customize/config/) — `markup.goldmark` 相关键
