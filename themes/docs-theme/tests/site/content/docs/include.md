...
{{</* include file="parts/install-docs.md" */>}}
```

The file it pulls in is ordinary Markdown living under `assets/`:

````markdown {title="assets/parts/install-docs.md"}
Installing Docs into an existing Hugo site takes three commands:

```sh
hugo mod init github.com/you/your-site
hugo mod get github.com/pgsty/docs
hugo server
```
...
the fragment above — "The current release is v0.5.0" — is its
...
````markdown {title="Source"}
```sh
hugo mod get github.com/pgsty/docs@{{</* param tdVersion.latest */>}}
```
...
[Release notes](https://github.com/pgsty/docs/releases/tag/{{</* param tdVersion.latest */>}})
````
...
[Configuration](https://docs.pgsty.com/docs/customize/config/); page parameters
are in [front matter](https://docs.pgsty.com/docs/write/frontmatter/).
...
- [Code blocks](/docs/code/) — every fence attribute, and the pipeline `include code=true` reuses
- [Tabs](/docs/tabs/) — per-platform or per-language fragments
- [Configuration](https://docs.pgsty.com/docs/customize/config/) — the site parameters `param` can reach
- [Front matter](https://docs.pgsty.com/docs/write/frontmatter/) — page parameters, which win over site configuration
