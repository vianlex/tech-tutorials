...
weight: 150
search_keywords: [ECharts, 图表, 统计图, 柱状图, 折线图, 饼图, chart, bar, line, pie, height, theme, DocsEchartsFunctions]
---
...
`radius` 给两个值就是环形图。下面是 Docs 的 29 个 shortcode 按用途的构成。
...
围栏是数据，不能带 JavaScript。某个选项需要函数时（提示框格式化、数据驱动的颜色），在选项里写字符串 `"$fn:名字"`，再把这个名字注册到 `window.DocsEchartsFunctions`：

````markdown {title="源码"}
<script>
  window.DocsEchartsFunctions = window.DocsEchartsFunctions || {};
  window.DocsEchartsFunctions.pageShare = function (params) {
    var p = params[0];
...
<script>
  window.DocsEchartsFunctions = window.DocsEchartsFunctions || {};
  window.DocsEchartsFunctions.starCount = function (p) {
    return Number(p.value).toLocaleString('zh-CN');
  };
  window.DocsEchartsFunctions.starColour = function (p) {
...
<script>
  window.DocsEchartsFunctions = window.DocsEchartsFunctions || {};
  window.DocsEchartsFunctions.phaseSeconds = function (params) {
...
数据经常变动（版本矩阵、发布物清单）时不做成图：改用[表格](/zh/docs/table/)，或[发布与下载页](https://docs.pgsty.com/zh/docs/write/releases/)中由 `data/` 驱动的组件。
