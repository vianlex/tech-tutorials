...
```markdown {title="源码"}
![Docs 文档外壳：侧栏、正文与目录三栏](/images/docs.webp)
```
...
| 全局资源 `assets/images/…` | `![…](images/logo/docs.webp)` | 多页共用、还要做处理（缩放 / 裁切）的图 |
| 静态目录 `static/images/…` | `![…](/images/docs.webp)` | 不需要处理的大图、下载物；主题拿不到尺寸时可以用 `width`/`height` 补 |
...
> [!NOTE]
> 块级图片依赖站点设置 `markup.goldmark.parser.wrapStandAloneImageWithinParagraph: false`（本站已配置；见[配置总览](https://docs.pgsty.com/zh/docs/customize/config/)）。缺少它时 Goldmark 会把独立图片包进 `<p>`，属性行也会被当作正文。
...
```markdown {title="源码"}
![Docs 文档站首页概览](/images/docs.webp)
{width="300" height="150" caption="static/images/ 里的 600×300 截图按一半显示"}
```
...
```markdown {title="源码"}
[![点击进入亮点特性页](/images/docs.webp)](https://docs.pgsty.com/zh/docs/about/features/)

![发布卡片](/images/releasenote.webp)
{caption="点击图片查看发布与下载页的说明" link="https://docs.pgsty.com/zh/docs/write/releases/"}
```
...
编号图用于书籍与长篇手册：属性行加 `num`，可选 `#id`。编号是作者书写的字符串（`2-1`、`3.4`），主题不自动计数；图注前加本地化的「图 2-1」前缀，`#id` 缺省为 `fig-<num>`。正文用普通链接 `[图 2-1](#fig-2-1)` 或 `xref` shortcode 引用；全书图目录见[书籍出版](https://docs.pgsty.com/zh/docs/write/book/)。
...
```markdown {title="源码"}
![侧栏（浅色）](/images/docs.webp)
{class="only-light"}

![侧栏（深色）](/images/docs.webp)
{class="only-dark"}
```
...
- 构建期不下载远程图片。
- 缩放不支持拖拽、平移、上一张 / 下一张；一组相关图片使用[画廊](/zh/docs/gallery/)。

## 相关 {#related}

- [画廊](/zh/docs/gallery/) — 一组图片共用一个缩放对话框
- [书籍出版](https://docs.pgsty.com/zh/docs/write/book/) — 图目录、`xref` 交叉引用
- [品牌外观](https://docs.pgsty.com/zh/docs/customize/brand/) — 站点 logo 与 favicon 放哪
- [卡片](/zh/docs/cards/) — 卡片上的图片
