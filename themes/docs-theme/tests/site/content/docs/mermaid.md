...
flowchart LR
  ...
  config["hugo.yaml"] --> Hugo
  theme["Docs theme"] --> Hugo
  Hugo --> site["public/"]
```
````
...
five states an Docs release passes through. They are not interchangeable, and a
...
[Configuration](https://docs.pgsty.com/docs/customize/config/); for accepted
values see the
...
`mermaid` fence; see [Steps](/docs/steps/).
