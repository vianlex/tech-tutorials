...
整站关掉复制用 `params.ui.code_copy: false`，它优先于每个块自己写的 `copy`（见[配置总览](https://docs.pgsty.com/zh/docs/customize/config/)）。复制按钮只有图标，成功与失败会换图标并播报本地化状态；复制内容保留缩进、空行与 Unicode，去掉行号，末尾只留一个换行。
...
`num` 与 `caption` 必须成对出现，只写一个会构建失败；`num` 与标签页属性 `tab` 互斥。图、表、公式的编号写法与索引见[书籍出版](https://docs.pgsty.com/zh/docs/write/book/)。
...
完整规则（分组语法、URL hash、跨组同步、正文标签页）在[标签页](/zh/docs/tabs/)。
...
| `tab` | 非空字符串 | 无 | 标签名，见[标签页](/zh/docs/tabs/)；与 `num` 互斥 |
...
- [标签页](/zh/docs/tabs/) — 相邻围栏合成标签页的完整规则
- [引用](/zh/docs/include/) — 把仓库里的真实文件当代码块插进来
- [书籍出版](https://docs.pgsty.com/zh/docs/write/book/) — 编号示例、交叉引用与示例目录
- [打印支持](https://docs.pgsty.com/zh/docs/customize/print/) — 长代码在打印里的形态
