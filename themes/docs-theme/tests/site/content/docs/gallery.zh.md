...
画廊（Gallery）把一组相关图片排成响应式网格，围栏里每行一张图。适用于同一件事的几个视图：几张截图、几种状态、几套配色。单张图用[图片](/zh/docs/image/)；相互之间没有顺序与对比关系的图片不适合放进同一个画廊。
...
````markdown {title="源码"}
```gallery
![Docs 的默认文档外壳](/images/docs.webp)
![上游 Docsy 的经典布局](/images/docsy.webp)
```
````
...
````markdown {title="源码"}
```gallery
![Docs 文档页面的三栏布局](/images/docs.webp) # 默认外壳：侧栏、正文、目录
![Docsy 的经典文档布局](/images/docsy.webp) # Docs 的上游 Docsy，内容模型一脉相承
...
````markdown {title="源码"}
```gallery
![Docs 的默认文档外壳](/images/docs.webp) # 点击进入「图片」组件页 {link=/zh/docs/image/}
![发布说明页面](/images/releasenote.webp) # 点击进入「发布与下载」 {link=https://docs.pgsty.com/zh/docs/write/releases/}
```
````
...
````markdown {title="源码"}
```gallery
![Docs 文档总览](/images/docs.webp) # 来自 static/images/…，原样发布
![Docs 发布说明](/images/releasenote.webp) # 与上一张图共用同一个简单目录
```
````
...
画廊没有自己的缩放运行时，复用整页共用的那个对话框。页面上没有可缩放的图时，运行时不加载。细节见[图片 · 缩放](/zh/docs/image/#zoom)。

## 加 class 与分标签页 {#class-and-tabs}

`class` 可以加在整个围栏上（写在语言后面）或某一项上（行尾），主题不解释它，原样透传给站点 CSS。围栏带 `tab=`（以及 `group=` `value=`）时成为一组[标签页](/zh/docs/tabs/)里的一页。

````markdown {title="源码"}
```gallery {tab="Docs" group="shell" value="docs"}
![Docs 的默认文档外壳](/images/docs.webp) # 侧栏、正文、目录
```
```gallery {tab="Docsy" value="docsy"}
![上游 Docsy 的经典布局](/images/docsy.webp) # 内容模型一脉相承
```
````
...
- [图片](/zh/docs/image/) — 单张图、图注、编号、缩放开关
- [卡片](/zh/docs/cards/) — 带图片的链接网格
- [标签页](/zh/docs/tabs/) — 按平台 / 主题并列多组图
- [文件树](/zh/docs/filetree/) — 行语法与画廊同源
