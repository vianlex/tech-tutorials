...
````markdown {title="Source"}
```diff {title="a change to hugo.yaml"}
 params:
   ui:
...
````markdown {title="Source"}
```yaml {title="hugo.yaml" collapse=8}
baseURL: https://docs.pgsty.com/
title: Docs
...
[Configuration](https://docs.pgsty.com/docs/customize/config/)). The copy button
...
````markdown {title="Source"}
```bash {tab="Homebrew" group="docs-install" value="brew"}
brew install hugo
```
...
in running text — are on the [Tabs](/docs/tabs/) page.
...
Inside the `{…}` after the language on the opening line, Docs's own attributes:
...
| `tab` | non-empty string | none | Tab label, see [Tabs](/docs/tabs/); mutually exclusive with `num` |
...
- [Tabs](/docs/tabs/) — the full rules for assembling adjacent fences
- [Include](/docs/include/) — pull a real file from the repository in as a code block
- [Publishing books](https://docs.pgsty.com/docs/write/book/) — numbered examples, cross references, the list of examples
- [Print](https://docs.pgsty.com/docs/customize/print/) — what long code looks like on paper
