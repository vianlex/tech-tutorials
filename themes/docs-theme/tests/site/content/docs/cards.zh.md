...
卡片（Cards）是一组并列的链接：每张卡片一个链接标题加一句描述，网格随容器宽度自适应。适合栏目首页、「接下来读什么」与几条并列路径的入口。不适合排版正文段落（用普通段落）或做图片墙（用[画廊](/zh/docs/gallery/)）。
...
```markdown {title="源码"}
- [快速上手](https://docs.pgsty.com/zh/docs/start/) — 克隆这个文档站，删掉不需要的页面，替换为你的站点信息。
- [创作内容](https://docs.pgsty.com/zh/docs/write/) — 页面怎么组织、front matter 有哪些键。
- [定制站点](https://docs.pgsty.com/zh/docs/customize/) — 导航、搜索、品牌、多语言。
{.cards}
```
...
```markdown {title="源码"}
- [页面参数](https://docs.pgsty.com/zh/docs/write/frontmatter/)

  每个页面参数在这里有唯一定义：类型、默认值、取值范围，以及讲它的那一页。

- [配置总览](https://docs.pgsty.com/zh/docs/customize/config/)
...
```markdown {title="源码"}
{{</* cards */>}}
{{</* card title="快速上手" link="https://docs.pgsty.com/zh/docs/start/" icon="fa-solid fa-rocket" badge="从这里开始" */>}}
Fork 文档站本身，十分钟内完成本地预览。
{{</* /card */>}}
{{</* card title="发布与下载页" link="https://docs.pgsty.com/zh/docs/write/releases/" icon="fa-solid fa-box-open" badge="v0.5" */>}}
一个发布 URL + 资产表 + 校验和，全部本地生成。
{{</* /card */>}}
{{</* card title="键盘导航" link="https://docs.pgsty.com/zh/docs/customize/keyboard/" icon="fa-solid fa-keyboard" */>}}
全站快捷键与焦点顺序。
{{</* /card */>}}
{{</* /cards */>}}
```
...
`hugo mod get github.com/pgsty/docs`。推荐方式，升级只需改一行版本号。
{{</* /card */>}}
...
```markdown {title="源码"}
{{</* cards */>}}
{{</* card title="Docs 文档外壳" link="https://docs.pgsty.com/zh/docs/about/features/" image="/images/docs.webp" image_alt="Docs 文档页面：侧栏、正文与目录三栏" */>}}
侧栏、正文、目录，三栏可以单独关闭。
{{</* /card */>}}
{{</* card title="发布说明" link="https://docs.pgsty.com/zh/docs/write/releases/" image="/images/releasenote.webp" decorative=true */>}}
...
卡片图片不参与[图片缩放](/zh/docs/image/#zoom)，整张卡片本身已经是链接。

## 栏目首页的自动卡片 {#section-index}

栏目首页（`_index.md`）不需要手写卡片列表：主题读子页的 `title`、`description`、`icon` 自动生成一组卡片。本站在 `hugo.yaml` 中全局启用：
...
自动卡片与手写卡片使用同一套 `td-content-card` 样式，区别只在数据来源。栏目首页不要手写子页清单：手写清单会与侧栏不同步。要排的内容不是本栏目的子页时（例如混合站外链接、跨栏目推荐），才在正文里手写卡片。相关键的完整定义见[配置总览](https://docs.pgsty.com/zh/docs/customize/config/)。
...
- 卡片不放长文：描述超过两行时改用正文段落或[提示块](/zh/docs/callout/)。

## 相关 {#related}

- [参数表](/zh/docs/fields/) — 同样有原生 / shortcode 两种形态
- [画廊](/zh/docs/gallery/) — 一组图片的网格
- [徽章](/zh/docs/badge/) — 行内状态标签
- [组织内容](https://docs.pgsty.com/zh/docs/write/organize/) — 栏目、权重与首页
- [配置总览](https://docs.pgsty.com/zh/docs/customize/config/) — `section_index` 等站点参数
