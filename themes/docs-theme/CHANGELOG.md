...
All notable changes to Docs are documented here. The project follows
[Semantic Versioning](https://semver.org/) for published tags.

## [1.0.0] - 2026-08-29

Docs 1.0.0 declares the current component, configuration, content, output, and
...
- The project README now leads with Docs Starter, records the capability and
...
- Hugo Themes gallery media now uses optimized 3:2 landing-page captures. Theme
  metadata uses the Docs wordmark, expands discovery tags and features, and
...
- The Hugo Module now declares Go 1.26 instead of 1.27. Docs contains no Go
...
- The project README now leads with Docs Starter, the current capability and
  ...
  footprint.
- Theme metadata uses the Docs wordmark, expands its discoverability tags and
...
- Upstream attribution in the page annotation. A site that vendors third-party
  documentation had to reimplement the notice itself: the two keys Docs
...
`bin/migrations/docs06.py report --sites <dir>` inventories a site and
...
- Make corpus/site build tools ignore only a vendored Docs copy while retaining
...
- Move public documentation, examples, tutorials, and case studies to
  `docs.pgsty.com`. Keep the checker suite's self-contained `tests/site/` as the
...
- **Breaking.** The `image` shortcode. Every image is now the Markdown image
  ...
  sources alike. `bin/migrations/docs06.py migrate --only image` rewrites
  existing calls.
...
- **Breaking.** The `{.filetree}` and `{.gallery}` list markers, the
  ...
  [component contract](https://docs.pgsty.com/docs/design/components/).
...
- Native Markdown forms for every content primitive, so most components no
  ...
  [component contract](https://docs.pgsty.com/docs/design/components/) is the
  authoring guide.
...
- A content migration toolkit, `bin/migrations/docs06.py`, with
...
- `bin/check-namespace.py`: every class the theme generates starts with
  ...
  `--td-`, and every JavaScript global with `Docs`. Third-party markup and the
...
- **Breaking.** Configuration keys converge on three rules ahead of the 1.0
  ...
  `manual_link_relref`; `bin/migrations/docs06.py migrate --only
...
- **Breaking.** One naming namespace. Classes the theme generates are `td-`
  ...
  `ul-N`, and the landing subsystem's `docs-*` set are gone); data attributes
  are `data-td-*`; CSS custom properties are `--td-*` (`--docs-*` and
  `--term-*` are gone); the ECharts extension point is
  `window.DocsEchartsFunctions`. The site header and nav are now
...
- `bin/migrations/docs06.py` gains a `frontmatter` transform (run first)
...
- The giscus palettes ship as `assets/css/giscus-{light,dark}.css` and are
  ...
  `/css/giscus-docs-*.css` itself (those files are gone). Set a giscus
...
- Preserve Docsy's `params.copyright` string/map contract and Hugo's top-level
  `copyright` fallback for the left side of the bottom bar. Replace Docs's
  ...
  `Powered by Docs`; an explicit empty string hides the center region. The
...
- Add validated `technical` and `system` typography presets plus public
  ...
  seed the new roles, and the system preset does not request Docs brand fonts.
...
- **Require explicit Algolia credentials.** Sites that enable
  `params.search.algolia` must provide `appId`, `apiKey`, and `indexName`. Docs
...
- Complete 89-key i18n schemas for every bundled locale, including a generic
  `zh` catalog; untranslated Docs-only labels use explicit English fallbacks.
...
- Public source comments for the Docs shell now describe behavior in English;
...
- Preserve Docs light and dark brand tokens when the stock Bootstrap RTL
...
- First reviewed Docs release after the Docsy fork, requiring Hugo Extended
  0.160.1 or newer.
...
[Unreleased]: https://github.com/pgsty/docs/compare/v1.0.0...HEAD
[1.0.0]: https://github.com/pgsty/docs/compare/v0.8.0...v1.0.0
[0.8.2]: https://github.com/pgsty/docs/compare/v0.8.1...v0.8.2
[0.8.1]: https://github.com/pgsty/docs/compare/v0.8.0...v0.8.1
[0.8.0]: https://github.com/pgsty/docs/compare/v0.7.1...v0.8.0
[0.7.1]: https://github.com/pgsty/docs/compare/v0.7.0...v0.7.1
[0.7.0]: https://github.com/pgsty/docs/compare/v0.6.1...v0.7.0
[0.6.1]: https://github.com/pgsty/docs/compare/v0.6.0...v0.6.1
[0.6.0]: https://github.com/pgsty/docs/compare/v0.5.0...v0.6.0
[0.5.0]: https://github.com/pgsty/docs/compare/v0.4.2...v0.5.0
[0.4.2]: https://github.com/pgsty/docs/compare/v0.4.1...v0.4.2
[0.4.1]: https://github.com/pgsty/docs/compare/v0.4.0...v0.4.1
[0.4.0]: https://github.com/pgsty/docs/compare/v0.3.0...v0.4.0
[0.3.0]: https://github.com/pgsty/docs/compare/v0.2.1...v0.3.0
[0.2.1]: https://github.com/pgsty/docs/compare/v0.2.0...v0.2.1
[0.2.0]: https://github.com/pgsty/docs/compare/v0.1.0...v0.2.0
[0.1.0]: https://github.com/pgsty/docs/releases/tag/v0.1.0
