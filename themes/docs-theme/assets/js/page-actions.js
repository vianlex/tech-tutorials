...
(function () {
  ...
  function syncUrl(control, id) {
    var url = window.DocsActions.resolveUrl(id);
  ...
  document.querySelectorAll('[data-td-action]').forEach(function (control) {
    ...
    var action = window.DocsActions.get(id);
    ...
    if (id === 'copy_markdown') {
      ...
      ['pointerenter', 'focus'].forEach(function (eventName) {
        control.addEventListener(eventName, function () {
          window.DocsActions.preloadMarkdown(action.url).catch(function () {});
        }, { once: true });
      });
      control.addEventListener('click', function () {
        window.DocsActions.run(id, { source: 'page' })
  ...
  function initTitleMenu() {
    ...
    function open() {
      if (window.DocsSurfaceCoordinator) {
        window.DocsSurfaceCoordinator.closeOthers('page-actions');
      }
    ...
    if (window.DocsSurfaceCoordinator) {
      window.DocsSurfaceCoordinator.register('page-actions', function () {
        close(false);
      });
    }
