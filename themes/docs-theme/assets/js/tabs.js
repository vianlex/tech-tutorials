  ...
  function groupAdjacentBlocks(doc, blocks) {
    ...
    runs.forEach(function (run, runIndex) {
      ...
      if (group && values.some(function (v) { return !v; })) {
        if (global.console && console.warn) console.warn('Docs tabs: group "' + group + '" needs a value on every block; switching locally.');
        group = '';
      }
      ...
      if (duplicates.length) {
        if (global.console && console.warn) console.warn('Docs tabs: duplicate values in an adjacent run: ' + duplicates.join(', '));
        return;
      }
